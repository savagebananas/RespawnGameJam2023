...
public class SpawnGhost4 : MonoBehaviour
{
    // Start is called before the first frame update
    public GameObject ghost;
    public int index = -1;
    public static GameObject[] locations4 = new GameObject[3];
    ...
    void Start()
    {
        for (int i = 1; i<=locations4.Length;i++) {
            string name = "GL" + (i+3);
            locations4[i-1] = GameObject.Find(name);
        }
    ...
    private Vector3 getRandomPosition() {
        if (index==-1) {
            ...
            return locations4[index].transform.position;
        }
        int temp = index;
        List<GameObject> tmp = new List<GameObject>();
        foreach (GameObject x in locations4) {
            if (x.GetComponent<GhostLocationRadius>().getShouldSpawn()) {
                tmp.Add(x);
            }
        }
        tmp.Remove(locations4[index]);
